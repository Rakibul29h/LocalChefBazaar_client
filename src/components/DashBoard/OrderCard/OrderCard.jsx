...
const OrderCard = ({ order }) => {
  const { orderStatus, mealName, cost, chefID, orderTime, quantity, chefName } =
    order;
  return (
        <div className=" flex flex-col justify-between  min-w-60 w-full bg-white rounded-2xl p-5 shadow-sm border border-gray-100 ">
          <div className="w-full">
            ...
            <div className="flex justify-between items-start gap-3 mb-5">
              ...
              <span className={` text-[10px] font-bold px-2.5 py-1 rounded tracking-wide uppercase ${orderStatus ==="pending"?"bg-orange-100 text-orange-600 ": orderStatus==="accepted"? "bg-blue-100 text-blue-700":"bg-green-100 text-green-300 "}`}>
                {orderStatus}
              </span>
            </div>
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="text-gray-900 font-bold">{new Date(orderTime).toLocaleTimeString('En-us')}</span>
              </div>
            </div>
          </div>
          ...
          {
             orderStatus === 'accepted' && <div className="bg-green-600 hover:bg-green-700 text-white font-medium text-center py-3 rounded-xl text-sm">
            Pay Now
          </div>
          }
          {/* {
