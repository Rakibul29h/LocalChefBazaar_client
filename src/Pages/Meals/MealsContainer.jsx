...
const MealsContainer = ({ sort, search,setCurrentPage,currentPage }) => {
  ...
  const { data: mealsData = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const result = await axiosSecure(
        `/allMeals?sort=${sort}&limits=${limit}&skip=${
          currentPage * limit
        }&search=${search}`
      );
  ...
  return (
    ...
    </div> :<span className="my-5 text-lg md:px-5 text-gray-500">No meals found.</span>
    }
    </>
